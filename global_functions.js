...
const { NotNullViolationError, ValidationError, UniqueViolationError, DBError, ForeignKeyViolationError } = require('objection');
  ...
  return promise
    .then(data => {
      ...
      if(err instanceof NotNullViolationError){
        err.message = `Not null constraint failed for column ${err.column}`;
        return [pe(err)]
      }else if(err instanceof UniqueViolationError){
        err.message = `Unique constraint failed for columns ${err.columns}`;
        return [pe(err)]
      }else if(err instanceof ForeignKeyViolationError){
        err.message = `Foreign Key constraint ${err.constraint} failed`;
        return [pe(err)]
      }else if(err instanceof DBError){
        err.message = `Some unknown DB error ${err.nativeError}`;
        return [pe(err)]
      }else{
        return [pe(err)]
      }});
